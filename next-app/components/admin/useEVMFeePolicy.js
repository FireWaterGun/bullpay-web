...
export default function useEVMFeePolicy() {
  ...
  async function loadSettings() {
    try {
      ...
      logger.error('Failed to load EVM fee policy settings:', error)
  ...
  async function handleSave(e) {
    ...
    try {
      ...
      logger.error('Failed to save EVM fee policy settings:', error)
  ...
  async function handleSaveChain() {
    try {
      ...
      logger.error('Failed to save chain setting:', error)
      toast.error(error?.message || t('admin.evm.saveError', { defaultValue: 'Failed to save settings' }))
    } finally {
      setLoading(false)
    }
