 ...
 */
export default function LocaleDatePicker({ value, onChange, locale = 'en-US', placeholder = '', className = '', style = {}, t }) {
          ...
          <div style={{ display: 'grid', gridTemplateColumns: 'repeat(7, 1fr)', textAlign: 'center' }}>
            {calendarDays.map((item, i) => {
              ...
              return (
                <div
                  key={i}
                  onClick={() => item.current && item.date && selectDate(item.date)}
                  style={{
                    padding: '6px 2px',
                    ...
                    color: !item.current ? '#ccc' : isSelected ? '#fff' : isToday ? '#696cff' : '#333',
                    backgroundColor: isSelected ? '#696cff' : 'transparent',
                    transition: 'background-color 0.15s',
                  }}
                  onMouseEnter={(e) => { if (item.current && !isSelected) e.currentTarget.style.backgroundColor = '#f0f0f0' }}
