...
export default function WebhookLogDetailPage() {
  ...
  const { data: log, isLoading, isValidating, mutate } = useApi(
    ...
    (token) => getUserWebhookLog(token, id),
    { onError: () => toast.error(t('webhookLog.loadError', { defaultValue: 'Failed to load webhook log' })) }
  )
  ...
  if (!log) {
    return (
      <div className="rounded-lg bg-warning-50 dark:bg-warning-950/30 text-warning-700 dark:text-warning-300 px-4 py-3 text-sm">
        {t('webhookLog.notFound', { defaultValue: 'Webhook log not found' })}
      </div>
    )
  }
    ...
    <>
              ...
              <tbody>
                ...
                <tr>
                  <td className="text-surface-500 py-2 pr-4">
                    {t('webhookLog.paymentId', { defaultValue: 'Payment ID' })}
                  </td>
                ...
                <tr>
                  <td className="text-surface-500 py-2 pr-4">
                    {t('webhookLog.callbackUrl', { defaultValue: 'Callback URL' })}
                  </td>
                  <td className="py-2">
              ...
              <tbody>
                <tr>
                  <td className="text-surface-500 py-2 pr-4 w-2/5">
                    {t('webhookLog.httpStatus', { defaultValue: 'HTTP Status' })}
                  </td>
      ...
      {log.requestPayload && (
        <Card className="mt-6">
          <div className="px-6 py-4 border-b border-surface-200">
            <h5 className="font-semibold text-surface-900 mb-0">
              {t('webhookLog.requestPayload', { defaultValue: 'Request Payload' })}
            </h5>
          </div>
          <div className="p-6">
            <pre className="bg-surface-50 dark:bg-dark-elevated rounded-lg p-4 mb-0 text-xs overflow-auto max-h-[400px]">
      ...
      {log.responseBody && (
        <Card className="mt-6">
          <div className="px-6 py-4 border-b border-surface-200">
            <h5 className="font-semibold text-surface-900 mb-0">
              {t('webhookLog.responseBody', { defaultValue: 'Response Body' })}
            </h5>
          </div>
          <div className="p-6">
            <pre className="bg-surface-50 dark:bg-dark-elevated rounded-lg p-4 mb-0 text-xs overflow-auto max-h-[400px]">
