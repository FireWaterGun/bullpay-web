      ...
      <div className="mb-4">
        <div className="mb-3">
          <h6 className="font-semibold mb-1 text-base">
            {t('admin.withdrawal.autoApprove', { defaultValue: 'Auto Approve' })}
          </h6>
          <p className="text-surface-500 mb-0 text-sm">
            {t('admin.withdrawal.autoApproveDesc', { defaultValue: 'Automatically approve small withdrawals' })}
          </p>
        </div>
          ...
          <Table responsive={false} className="mb-0">
            <tbody>
              <tr className="bg-surface-100 dark:bg-white/[0.03]">
                <td width="35%" className="py-3 pl-3 text-sm">
                  {t('admin.withdrawal.enabled', { defaultValue: 'Enabled' })}
                </td>
                <td className="py-3">
                  <div className="flex items-center gap-2">
                      ...
                      onChange={async (newEnabled) => {
                        ...
                        try {
                          await upsertSetting(token, {
                            keyName: 'withdrawal.auto_approve.enabled',
                            value: String(newEnabled),
                          })
                          toast.success(
                            t('admin.withdrawal.saveSuccess', { defaultValue: 'Settings saved successfully' })
                          )
                        } catch (err) {
                          setAutoApprove(prev)
                          toast.error(
                            err?.message || t('admin.withdrawal.saveError', { defaultValue: 'Failed to save settings' })
                          )
                        } finally {
                          setToggling(false)
                        }
                    ...
                    />
                    <span
                      className={`text-xs font-semibold ${autoApprove.enabled ? 'text-success' : 'text-surface-400'}`}
                    >
              ...
              <tr>
                <td className="py-3 pl-3 text-sm">
                  {t('admin.withdrawal.thresholdUsd', { defaultValue: 'Threshold (USD)' })}
                </td>
                <td className="py-3">
                  <div className="flex items-center gap-2">
