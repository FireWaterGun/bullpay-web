...
function sourceBadge(source) {
  if (source === 'granted') return { color: 'success', labelKey: 'granted' }
  if (source === 'denied') return { color: 'danger', labelKey: 'denied' }
  return { color: 'secondary', labelKey: 'default' }
}
...
export default function PermissionGroupCard({
  ...
  return (
    <Card className="mb-3">
      <div className="px-5 py-3 border-b border-surface-200 cursor-pointer" onClick={() => onToggle(group)}>
        <div className="flex items-center justify-between">
          ...
          <div className="flex items-center gap-2">
            <small className="text-surface-500">
              {activeInGroup}/{perms.length} {t('admin.roles.active', { defaultValue: 'active' }).toLowerCase()}
            </small>
            <Badge color={color} label className="rounded-full text-[0.7rem]">
              {perms.length}
            </Badge>
          </div>
        </div>
                    ...
                    <div className="flex items-center gap-2 flex-wrap">
                      ...
                      {isOverridden && (
                        <Badge color="warning" label className="text-[0.6rem]">
                          <i className="bx bx-edit-alt text-[0.55rem]"></i>{' '}
                          {t('admin.roles.override', { defaultValue: 'override' })}
                        </Badge>
                      )}
                    </div>
                      ...
                      <div className="flex gap-1">
                        {isOverridden && (
                          <Button
                            onClick={(e) => {
                              e.stopPropagation()
                              onRevert(override.id, p.permission)
                            }}
                            title={t('admin.roles.revertToDefault', { defaultValue: 'Revert to default' })}
                            size="sm"
                            className="border border-warning-500 text-warning-500 bg-transparent hover:bg-warning-500 hover:text-white py-[0.15rem] px-[0.5rem] text-[0.7rem]"
                          >
                            <i className="bx bx-undo mr-1"></i>
                            {t('admin.roles.revert', { defaultValue: 'Revert' })}
                          </Button>
                        )}
                        {p.active ? (
                          <Button
                            ...
                            size="sm"
                            className="border border-danger-500 text-danger-500 bg-transparent hover:bg-danger-500 hover:text-white py-[0.15rem] px-[0.5rem] text-[0.7rem]"
                          >
                          ...
                          <Button
                            onClick={(e) => {
                              e.stopPropagation()
                              onGrant(p.permission)
                            }}
                            title={t('admin.roles.grantPermission', { defaultValue: 'Grant this permission' })}
                            size="sm"
                            className="border border-success-500 text-success-500 bg-transparent hover:bg-success-500 hover:text-white py-[0.15rem] px-[0.5rem] text-[0.7rem]"
                          >
