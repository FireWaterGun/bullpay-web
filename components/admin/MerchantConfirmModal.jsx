...
export default function MerchantConfirmModal({ merchant, action, loading, onConfirm, onClose }) {
        ...
        <div className="bg-card rounded-xl shadow-xl">
          <div className="flex items-center justify-between p-5 border-b border-surface-200">
            <h5 className="text-lg font-semibold text-surface-800">
              <i
                className={`bx ${action === 'activate' ? 'bx-check-circle text-primary' : 'bx-block text-danger'} mr-2`}
              ></i>
              {action === 'activate'
          ...
          <div className="p-5">
            ...
            {action === 'activate' ? (
              <Alert variant="primary" className="py-2 mb-0">
                ...
                {t('admin.merchants.activateConfirm', {
                  defaultValue: 'Are you sure you want to activate this merchant?',
                })}
              </Alert>
            ) : (
              <Alert role="alert" className="py-2 mb-0">
                <i className="bx bx-error mr-1"></i>
                {t('admin.merchants.suspendConfirm', {
                  defaultValue:
                    'Are you sure you want to suspend this merchant? They will lose access to merchant features.',
                })}
              </Alert>
            )}
          </div>
