            ...
            {loading ? (
              ...
              <div className="overflow-x-auto">
                <Table responsive={false} className="mb-0">
                  <thead>
                    <tr>
                      <th className="uppercase font-semibold text-surface-500 text-[0.8rem]">
                        {t('admin.coin', { defaultValue: 'Coin' })}
                      </th>
                      <th className="text-right uppercase font-semibold text-surface-500 text-[0.8rem]">
                        {t('admin.revenue', { defaultValue: 'Revenue' })}
                      </th>
                      <th className="text-right uppercase font-semibold text-surface-500 text-[0.8rem]">
                        {t('admin.cost', { defaultValue: 'Cost' })}
                      </th>
                      <th className="text-right uppercase font-semibold text-surface-500 text-[0.8rem]">
                        {t('admin.operatingProfit', { defaultValue: 'Operating Profit' })}
                      </th>
                      <th className="text-right uppercase font-semibold text-surface-500 text-[0.8rem]">
                        {t('admin.margin', { defaultValue: 'Margin' })}
                      </th>
                    </tr>
                  </thead>
                  <tbody>
                    {byCoinData.length === 0 ? (
                      ...
                      <>
                        {byCoinData.map((item) => {
                          ...
                          return (
                            <tr key={`${item.coinSymbol}-${item.networkName || 'all'}`}>
                              ...
                              <td
                                className={`text-right ${profit > 0 ? 'text-success' : profit < 0 ? 'text-danger' : ''}`}
                              >
                        ...
                        <tr className="font-bold">
                          ...
                          <td
                            className={`text-right ${totals.profit > 0 ? 'text-success' : totals.profit < 0 ? 'text-danger' : ''}`}
                          >
          ...
          <div className="p-5 py-3">
            <div className="flex flex-wrap justify-between gap-3">
              <div>
                ...
                <span className="font-medium mr-3">Sweep {formatCurrency(summary?.totalSweepVolumeUsd || 0)}</span>
              ...
              <div>
                ...
                <span className="font-medium mr-3">{summary?.counts?.sweeps || 0} sweeps</span>
                <span className="text-surface-500">|</span>
                <span className="font-medium mx-3">{summary?.counts?.withdrawals || 0} withdrawals</span>
                <span className="text-surface-500">|</span>
                <span className="font-medium mx-3">{summary?.counts?.gasTopups || 0} gas topups</span>
              </div>
            </div>
          </div>
        </Card>
