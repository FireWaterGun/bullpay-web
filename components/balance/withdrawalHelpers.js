...
export const NETWORK_LABELS = {
  1: 'Bitcoin', 2: 'Lightning', 10: 'Ethereum', 11: 'ERC-20',
  20: 'BSC (BEP-20)', 21: 'BEP-20', 30: 'TRON (TRC-20)', 31: 'TRC-20',
  40: 'Polygon', 50: 'Solana', 60: 'TON', 61: 'TON (Jetton)',
  70: 'Base', 80: 'Arbitrum', 90: 'Optimism', 100: 'Avalanche C-Chain',
}
...
export function addressStatusBadgeClass(s) {
  ...
  const v = String(s || '').toLowerCase()
  if (v === 'pending_verification') return `${base} bg-amber-100 text-amber-700 dark:bg-amber-900/30 dark:text-amber-400`
  ...
  if (v === 'deleted') return `${base} bg-surface-100 text-surface-600 dark:bg-dark-elevated dark:text-dark-text-secondary`
...
export function statusBadgeClass(s) {
  ...
  if (v === 'PROCESSING' || v === 'APPROVED') return `${base} bg-blue-100 text-blue-700 dark:bg-blue-900/30 dark:text-blue-400`
  if (v === 'COMPLETED' || v === 'SUCCESS') return `${base} bg-green-100 text-green-700 dark:bg-green-900/30 dark:text-green-400`
  if (v === 'FAILED' || v === 'REJECTED' || v === 'ERROR') return `${base} bg-red-100 text-red-700 dark:bg-red-900/30 dark:text-red-400`
  if (v === 'CANCELLED' || v === 'CANCELED') return `${base} bg-surface-100 text-surface-600 dark:bg-dark-elevated dark:text-dark-text-secondary`
  return `${base} bg-surface-100 text-surface-600 dark:bg-dark-elevated dark:text-dark-text-secondary`
}
