...
export default function WalletEdit() {
  ...
  if (!wallet) {
    return (
      <div className="rounded-lg bg-danger-50 dark:bg-danger-950/30 text-danger-700 dark:text-danger-400 p-4">
        {t('wallets.notFound', { defaultValue: 'Wallet not found' })}
      </div>
    )
  }
          ...
          <Card>
            <div className="p-6">
              <form onSubmit={handleSave}>
                <div className="mb-3">
                  <Label>
                    {t('wallets.label', { defaultValue: 'Label' })} <span className="text-danger-500">*</span>
                  </Label>
                  <Input
                    type="text"
                    value={form.label}
                ...
                <div className="mb-3">
                  ...
                  <Input type="text" value={form.address} disabled className="font-mono" />

                  <small className="text-surface-500">
                    {t('wallets.addressNoEdit', { defaultValue: 'Address cannot be changed' })}
                  </small>
                </div>
                  ...
                  <Button type="submit" disabled={saving} className="flex-grow">
                    {saving ? (
                      <>
                        ...
                        {t('wallets.saving', { defaultValue: 'Saving...' })}
                      </>
                    ) : (
                      <>
                        <i className="bx bx-check mr-1"></i>
                        {t('wallets.save', { defaultValue: 'Save Changes' })}
                      </>
                    )}
                  </Button>
                  <Button
