      ...
      <div className="border border-surface-200 rounded-lg p-3">
        ...
        <div className="flex justify-between mb-2">
          ...
          <span className="text-sm font-medium">
            {feeEstimate.display?.grossAmount ||
              feeEstimate.display?.amount ||
              `${formatCoinAmount(fromRaw(feeEstimate.amountRaw, feeEstimate.decimals), 4)} ${sym}`}
          </span>
        </div>
        <div className="flex justify-between mb-2">
          <span className="text-sm">{t('balance.networkFee', { defaultValue: 'Network fee' })}</span>
          <span className="text-sm">
            {feeEstimate.display?.baseFee ||
              `${formatCoinAmount(fromRaw(feeEstimate.baseFeeRaw, feeEstimate.decimals), 4)} ${sym}`}
          </span>
        </div>
        <div className="flex justify-between mb-2">
          <span className="text-sm">
            {t('balance.platformFee', { defaultValue: 'Platform fee' })} (
            {feeEstimate.display?.percentFeeText || `${feeEstimate.feePercentage}%`})
          </span>
          <span className="text-sm">
            {feeEstimate.display?.percentFee ||
              `${formatCoinAmount(fromRaw(feeEstimate.percentFeeRaw, feeEstimate.decimals), 4)} ${sym}`}
          </span>
        </div>
        <div className="flex justify-between mb-2 pt-2 border-t border-surface-200">
          ...
          <div className="text-right">
            <div className="text-sm font-medium">
              {feeEstimate.display?.totalFee ||
                `${formatCoinAmount(fromRaw(feeEstimate.totalFeeRaw, feeEstimate.decimals), 4)} ${sym}`}
            </div>
        ...
        <div className="flex justify-between pt-2 border-t border-surface-200">
          ...
          <div className="text-right">
            <div className="font-semibold">
              {feeEstimate.display?.netAmount ||
                `${formatCoinAmount(fromRaw(feeEstimate.netAmountRaw, feeEstimate.decimals), 4)} ${sym}`}
            </div>
