      ...
      >
        <div className="text-center px-6 py-8">
          ...
          <div className="mb-3">
            ...
            <div className="font-bold text-3xl text-surface-900">{receiveAmount}</div>
          </div>
          ...
          <div className="text-left mb-4 rounded-xl p-5 border border-surface-200">
            <div className="flex justify-between items-center mb-3">
              <span className="text-surface-500 text-[0.9rem]">
                {t('balance.address', { defaultValue: 'Address' })}
              </span>
              <div className="flex items-center gap-2">
                ...
                >
                  <i
                    className={`bx ${copied ? 'bx-check text-success-600 dark:text-success-400' : 'bx-copy'} text-[1.1rem]`}
                  ></i>
                </button>
              </div>
            </div>
            <div className="flex justify-between items-center mb-3">
              <span className="text-surface-500 text-[0.9rem]">
                {t('balance.network', { defaultValue: 'Network' })}
              </span>
          ...
          <Button type="button" onClick={onClose} className="w-full py-2 font-semibold rounded-lg">
...
export function ErrorModalWrapper({ open, onClose, message, t }) {
  return (
    <ConfirmModal
      ...
      message={
        <div>{message || t('balance.withdrawErrorMsg', { defaultValue: 'Failed to process withdrawal request.' })}</div>
      }
