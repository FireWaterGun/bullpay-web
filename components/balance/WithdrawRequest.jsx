...
export default function WithdrawRequest() {
  ...
  useEffect(() => {
    ...
    ;(async () => {
      try {
        ...
        if (!mounted) return
        const balanceItem = Array.isArray(balRes?.breakdown) && balRes.breakdown.length > 0 ? balRes.breakdown[0] : null
  ...
  const canSubmit =
    amountNum > 0 &&
    amountNum <= available &&
    effectiveAddress.trim().length > 0 &&
    selectedWallet?.id &&
    feeEstimate &&
    !estimatingFee &&
    !is2FALoading
  ...
  return (
    <>
      {loading ? (
        <Card>
          <div className="p-6">
            <div className="animate-pulse space-y-3">
              <div className="h-4 bg-surface-200 dark:bg-dark-elevated rounded w-1/3"></div>
              <div className="h-4 bg-surface-200 dark:bg-dark-elevated rounded w-2/3"></div>
            </div>
          </div>
        </Card>
      ) : error ? (
        <div
          className="rounded-lg bg-danger-50 dark:bg-danger-950/30 text-danger-700 dark:text-danger-400 p-4"
          role="alert"
        >
          {error}
        </div>
        ...
        <div
          className="rounded-lg bg-warning-50 dark:bg-warning-950/30 text-warning-700 dark:text-warning-400 p-4"
          role="alert"
        >
          {t('common.noData') || 'Not found'}
        </div>
        ...
        <Card className="mx-auto max-w-[520px]">
          <div className="p-6 text-center py-10">
            ...
            <h5 className="mb-2 font-semibold text-lg">
              {t('balance.require2FATitle', { defaultValue: 'Two-Factor Authentication Required' })}
            </h5>
            <p className="text-surface-500 mb-4">
              {t('balance.require2FADesc', {
                defaultValue:
                  'For your security, please enable Two-Factor Authentication (2FA) before making withdrawals.',
              })}
            </p>
            ...
            {wallets.length === 0 || matchingWallets.length === 0 ? (
              <div className="text-center py-3">
                <h6 className="mb-2 font-semibold">
                  {t('wallet.requiredWithdrawTitle', { defaultValue: 'Withdrawal address required' })}
                </h6>
                <p className="text-surface-500 mb-3">
                  {t('wallet.requiredWithdrawDesc', {
              ...
              <>
                <div className="mb-3">
                  ...
                  <div className="flex items-center justify-between border border-surface-200 rounded-lg p-3">
                    ...
                    {networkLabel && (
                      <span className="inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-medium bg-danger-100 dark:bg-danger-900/30 text-danger-700 dark:text-danger-400">
                        {t('wallet.colNetwork', { defaultValue: 'Network' })}
                      </span>
                    )}
                  </div>
                </div>
                    ...
                    <Input
                      ...
                      onChange={(e) => {
                        ...
                        if (value === '' || (numValue >= 0 && numValue <= available)) {
                          ...
                          setAmountError(
                            t('balance.amountExceedsBalance', { defaultValue: 'Amount exceeds available balance' })
                          )
                        }
                      }}
                      placeholder="0.0"
                      className="text-lg pr-[80px]"
                    />
                ...
                {estimatingFee && !feeEstimate && (
                  <div className="mb-3 text-center">
                    ...
                    <span className="text-sm text-surface-500">
                      {t('balance.calculatingFee', { defaultValue: 'Calculating fee...' })}
                    </span>
                  </div>
                )}
                ...
                {feeError && !estimatingFee && (
                  <div className="mb-3">
                    <div
                      className="rounded-lg bg-danger-50 dark:bg-danger-950/30 text-danger-700 dark:text-danger-400 py-2 px-3"
                      role="alert"
                    >
                      <i className="bx bx-error-circle mr-1"></i>
      ...
      <SuccessModalWrapper
        ...
        receiveAmount={feeEstimate?.display?.netAmount || effectiveAmount}
        sym={sym}
      ...
      <Verify2FAModal
        ...
        description={t('balance.confirm2FADescription', {
          defaultValue: 'Enter your 2FA code to confirm this withdrawal',
        })}
        skipVerify={true}
      />
    </>
