...
export function Setup2FAModal({ show, onClose, onSuccess, token }) {
  ...
  const handleDownloadBackupCodes = () => {
    ...
    const footer = t('settings.2fa.backupCodesKeepSafe', {
      defaultValue: 'Keep these codes safe. Each code can only be used once.',
    })
  ...
  const handleEnable = async () => {
    ...
    try {
      ...
      const retryAfter = err?.retryAfterSeconds || err?.data?.retryAfterSeconds || err?.data?.error?.retryAfterSeconds
      ...
      <div className="bg-card rounded-xl shadow-xl mx-4 w-full max-w-[500px]">
        ...
        <div className="p-6">
          ...
          {loading && !setupData ? (
            ...
            <div className="rounded-lg bg-danger-50 dark:bg-danger-500/10 text-danger-700 dark:text-danger-300 p-4">
              {error}
            </div>
          ) : (
            <>
                ...
                <div className="text-center">
                  ...
                  <div className="mb-3">
                    ...
                    <InputGroup className="max-w-[300px] mx-auto">
                      <Input type="text" value={setupData.secret} readOnly className="text-center font-mono text-sm" />
        ...
        <div className="px-6 py-4 border-t border-surface-200 flex flex-wrap items-center gap-2">
          ...
          {step < 3 ? (
            <Button type="button" onClick={() => setStep((s) => s + 1)} disabled={loading || !setupData}>
              {t('common.continue', { defaultValue: 'Continue' })}
              <i className="bx bx-chevron-right ml-1"></i>
            </Button>
          ) : (
            <Button type="button" onClick={handleEnable} disabled={loading || totpCode.length !== 6 || countdown > 0}>
              {loading ? (
                <>
