  ...
  return (
    ...
    >
      <div
        role="dialog"
        aria-modal="true"
        aria-labelledby="merchant-action-title"
        className="bg-card rounded-xl shadow-xl w-full max-w-xl max-h-[85vh] overflow-y-auto"
        ...
        <form onSubmit={handleSubmit}>
          <div className="px-6 py-4 border-b border-surface-200 flex justify-between items-center">
            ...
            <button
              ...
              className="cursor-pointer text-surface-500 hover:text-surface-700 dark:text-surface-400 dark:hover:text-surface-200 text-xl leading-none"
              onClick={onClose}
              disabled={loading}
            >
          ...
          <div className="p-6">
            ...
            {action === 'rotate-secret' ? (
              <div
                className="rounded-lg bg-info-50 dark:bg-info-950/30 text-info-700 dark:text-info-400 py-2 px-3 mb-3"
                role="alert"
              >
                <i className="bx bx-info-circle mr-1"></i>
                {t('merchant.rotateConfirm', {
                  defaultValue:
                    'This will generate a new API secret. Your API key will remain the same. The old secret will stop working immediately.',
                })}
              </div>
            ) : (
              <div
                className="rounded-lg bg-danger-50 dark:bg-danger-950/30 text-danger-700 dark:text-danger-400 py-2 px-3 mb-3"
                role="alert"
              >
                <i className="bx bx-error mr-1"></i>
                {t('merchant.regenerateConfirm', {
                  defaultValue:
                    'This will generate a new API key AND secret. All existing credentials will be invalidated immediately.',
                })}
              </div>
            )}
            ...
            {error && (
              <div
                className="rounded-lg bg-danger-50 dark:bg-danger-950/30 text-danger-700 dark:text-danger-400 py-2 px-3 text-sm mt-3"
                role="alert"
              >
            ...
            <Button type="submit" disabled={!canSubmit}>
              {loading ? (
                <>
                  ...
                  {t('merchant.processing', { defaultValue: 'Processing...' })}
                </>
              ) : action === 'rotate-secret' ? (
