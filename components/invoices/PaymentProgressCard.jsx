...
export default function PaymentProgressCard({ invoice, t }) {
  ...
  const steps = [
    ...
    {
      key: 'awaiting',
      label: t?.('invoices.awaitingPayment', { defaultValue: 'Awaiting Payment' }) || 'Awaiting Payment',
      done: status !== 'pending' && status !== 'created',
    },
    {
      key: 'received',
      label: t?.('invoices.paymentReceived', { defaultValue: 'Payment Received' }) || 'Payment Received',
      done: status === 'paid' || status === 'completed' || status === 'partially_paid',
    },
    {
      key: 'completed',
      label: t?.('invoices.completed', { defaultValue: 'Completed' }) || 'Completed',
      done: status === 'paid' || status === 'completed',
    },
  ]
    ...
    <Card className="mb-3">
      <div className="p-5">
        ...
        <div className="flex justify-between text-sm text-surface-500 mb-3">
          <span>
            {amountReceived} / {amountRequired} {invoice?.coinSymbol || ''}
          </span>
