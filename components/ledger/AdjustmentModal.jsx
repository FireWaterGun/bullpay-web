        ...
        <div className="bg-card rounded-xl shadow-xl">
          ...
          <div className="p-5">
            ...
            <Alert role="alert" variant="warning" className="flex items-start mb-4">
              ...
              <div>
                ...
                <p className="mb-0 mt-1 text-sm">
                  {t('admin.adjustment.warningText', {
                    defaultValue:
                      'This action directly modifies system wallet balances. Please verify all details carefully before submitting.',
                  })}
                </p>
              </div>
            </Alert>
              ...
              {loadingWallets ? (
                <div className="flex items-center gap-2 py-2">
                  ...
                  <span className="text-surface-500 text-sm">
                    {t('admin.adjustment.loadingWallets', { defaultValue: 'Loading wallets...' })}
                  </span>
                </div>
              ) : wallets.length === 0 ? (
                ...
                >
                  {wallets.map((w) => (
                    <option key={w.key} value={w.key}>
                      {w.walletName} — {w.coinSymbol} ({w.networkName}) —{' '}
                      {t('admin.adjustment.balance', { defaultValue: 'Balance' })}: {w.confirmedBalance}
                    </option>
                  ))}
                </Select>
              )}
            ...
            <div className="mb-3">
              ...
              {amountTrimmed && !isValidAmount && (
                <small className="text-danger">
                  {t('admin.adjustment.invalidAmount', {
                    defaultValue: 'Enter a valid positive number (e.g. 1.5, 100)',
                  })}
                </small>
              )}
            </div>
            ...
            <div className="mb-3">
              ...
              <Input
                rows={3}
                placeholder={t('admin.adjustment.reasonPlaceholder', {
                  defaultValue: 'Describe why this adjustment is needed (min 10 characters)',
                })}
                value={reason}
              ...
              <small
                className={`${reasonTrimmed.length > 0 && reasonTrimmed.length < 10 ? 'text-danger' : 'text-surface-500'}`}
              >
            ...
            <div className="mb-3">
              <Label>
                {t('admin.adjustment.txHash', { defaultValue: 'Tx Hash' })}{' '}
                <span className="text-surface-500 text-sm">
                  ({t('admin.adjustment.optional', { defaultValue: 'optional' })})
                </span>
              </Label>
              <Input
                type="text"
          ...
          <div className="flex items-center justify-end gap-2 p-5 border-t border-surface-200">
            ...
            <Button type="button" onClick={handleSubmit} disabled={!isValid || disabled}>
              {submitting ? (
                <>
