...
export default function Verify2FAModal({ show, onClose, onSuccess, title, description, skipVerify = false }) {
            ...
            <div className="flex justify-center gap-1.5 sm:gap-2 mb-3" onPaste={handlePaste}>
              {[0, 1, 2, 3, 4, 5].map((i) => (
                <Input
                  ...
                  autoFocus={i === 0}
                  className="text-center font-bold w-10 h-11 !px-0 text-lg sm:w-11 sm:h-12 sm:text-xl"
                />
              ))}
            </div>
          )}
